...
final class UsageStore: ObservableObject {
    ...
    @Published private(set) var feishuNotificationsEnabled = UserDefaults.standard.bool(
        forKey: UsageStore.feishuNotificationsEnabledKey
    )
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            DispatchQueue.main.async {
                ...
                if isAutomaticSwitch {
                    ...
                    let targetIsEligible = CodexAutomaticSwitchPolicy.preferredCandidate(
    ...
    func start() {
        ...
        accountActions.recoverPendingSwitchIfNeeded { [weak self] result in
            ...
            case .success(.noPendingSwitch):
                break
            case let .success(.restoredOriginalAuth(reopened)):
                self.accountManagerMessage = reopened
                    ? "检测到上次切换中断；已恢复原账号并重新打开 Codex"
                    : "检测到上次切换中断；已恢复原账号"
            case let .success(.originalAuthAlreadyPresent(reopened)):
                self.accountManagerMessage = reopened
...
func debugLog(_ message: String) {
    ...
    let url = URL(fileURLWithPath: "/tmp/codex-account-manager-next.log")
