...
- 当前 `Windows` 现状：
  ...
  - 当前可见全局导航为 `Dashboard / AI Leadership / Threads`；Usage/Projects 的可见主入口收敛到 Dashboard 下方 tablist。
...
| --- | --- | --- | --- |
| TODO-01 | `complete` | 前端已实现固定上方 Dashboard + 下方 Tasks / Usage / Projects / Skills 四格切换；Leadership 保持独立明细。可见全局导航仅保留 Dashboard / AI Leadership / Threads。原生证据已完成。 | 不改 `useUsage()` 返回体或数据源。 |
| TODO-02 | `complete` | Usage/Projects 复用为下方真实面板，是其唯一可见 Dashboard 主入口；紧凑原生窗口证据已完成。 | 不新增计算字段，不替换统计/图表口径。 |
...
| TODO-06 | `evidence split` | 导航去重后的 Default Dashboard 与 AI Leadership 已更新原生重拍，showcase 和 parity report 已按同一结论更新。 | 下方四图保留为最近似上下文，不是当前顶层导航或严格跨平台一一对应证据。 |
...
- 导航去重后更新的 Default 原生紧凑窗口约为 `960x758 CSS px`：DWM `1439x1136` physical、DPI 144。三个顶部组为水平布局，L1-L7 标题、徽章、bar 与 labels 完整可见；下方四图的 `1440x1137` 历史批次仅保留为最近似上下文。
- 固定 top 的 Token mix 只显示 `Input`、`Cached input`、`Output` 各一次；不含重复计数，也不含 raw thread/project/tool 内容。
- 当前可见全局导航只显示 `Dashboard / AI Leadership / Threads`；Usage/Projects 没有第二个顶层可见入口。
...
updated Default Dashboard 与 AI Leadership 截图来自最终 `windows/target/release/codexu-tauri.exe` 的目标 HWND `PrintWindow` capture，不是 Computer Use。它们在可见导航去重后重拍：default DWM `1439x1136` physical、DPI 144，约 `960x758 CSS px`，明确显示三个全局导航项及完整 L1-L7。Computer Use Node runtime 初始化失败，错误为 `os error 3`，因此不得称为 Computer Use 验收通过。目标 release process 已在 capture 后清理，匹配进程为 0。
...
| Tasks | [dashboard-lower-panels-tasks-native.png](../../reports/assets/dashboard-lower-panels-tasks-native.png)（retained nearest-context image，不是当前全局导航证据） |
| Usage | [dashboard-lower-panels-usage-native.png](../../reports/assets/dashboard-lower-panels-usage-native.png)（retained nearest-context image，不是当前全局导航证据） |
| Projects | [dashboard-lower-panels-projects-native.png](../../reports/assets/dashboard-lower-panels-projects-native.png)（retained nearest-context image，不是当前全局导航证据） |
| Skills | [dashboard-lower-panels-skills-native.png](../../reports/assets/dashboard-lower-panels-skills-native.png)（retained nearest-context image，不是当前全局导航证据） |

验证事实：

- `npm run build`（`windows/apps/codexu-tauri/web`）通过。
- `cargo test --workspace`（`windows`）通过：9 passed / 0 failed；三个 zero-test targets。
- `cargo tauri build --no-bundle`（`windows/apps/codexu-tauri/src-tauri`）在 02:59:09 Asia/Shanghai 通过，产出 release executable。
- `git diff --check` 通过。

### 2026-07-28 comparison-evidence clarification（不改变上述实施状态或数据边界）

- `AI Leadership` 在 macOS/Windows 比较中属于 `fixed overview` 的层级对应；其 detail 是 drill-down，不是四个下方 variable panels 之一。Windows local Month value estimate 与 macOS 月度 Wool 缺少严格一一对应证据，且 Token mix 口径不同；不得把 estimate 作为完整 Wool 替代。
- `Tasks` 与 `Skills` 在严格 macOS/Windows 对比中分类为 `variable panel + not implemented`。Windows UI shell 和 retained 截图只提供最近似上下文/近似证据，不能证明严格同功能实现；该标签只描述比较证据边界，不改变本 TODO 的 UI shell 完成状态，也不是 Reader、IPC、数据契约、源码或产品范围的全局结论。
- 本轮导航去重后只有 default/Leadership 图完成更新原生重拍；下方四图因真实窗口在采集时收回 tray/minimized 而保留为旧的最近似上下文图，不能用于证明当前顶层导航。
