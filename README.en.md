...
- Today's observed inference performance: complete model calls with `model`, `reasoning_effort`, and incremental output tokens are identified only from today's updated local Codex rollouts, then grouped by model and effort for average, P50, P90, and effective throughput. Bubble tooltips disclose that throughput uses all output tokens divided by full duration, plus the reasoning-token count and share of total output. Prompt and response text are not exposed, and the metric is not labeled as TTFT or visible-text decode TPS.
