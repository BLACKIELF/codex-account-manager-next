...
- Adds an on-device model × reasoning-effort scatter plot above Codex Today's Tasks. X is P50 full-call duration with a whisker to P90, Y is output tokens divided by full-call duration, and bubble area represents call count. It compares today's observed experience without presenting the metric as TTFT or visible-text decode TPS.
...
- Today's observed inference performance: complete model calls with `model`, `reasoning_effort`, and incremental output tokens are identified only from today's updated local Codex rollouts, then grouped by model and effort for average, P50, P90, and effective throughput. Prompt and response text are not exposed, and the metric is not labeled as TTFT or visible-text decode TPS.
