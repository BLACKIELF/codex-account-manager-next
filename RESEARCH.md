...
> 本文是上游额度/统计读取研究快照。Next 的账号切换会在本机读取 `auth.json` 以校验身份并原子替换凭据，官方资料卡也可能访问 `chatgpt.com/backend-api/wham/profiles/me`；当前边界以 `SECURITY.md` 为准。
...
- Usage/statistics parsing does not expose credential values, but account management reads local `auth.json` only for identity validation and explicit switching.
- Official profile metadata may be requested from the fixed `chatgpt.com/backend-api/wham/profiles/me` endpoint; responses remain local.
- Session parsing does not retain prompt or tool payloads; it filters only the structured fields required for local metrics.
