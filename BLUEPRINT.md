...
> 本文保留上游统计/界面架构，不能覆盖 Next 新增的账号切换与飞书边界。当前安全与网络权威以 `README.md`、`SECURITY.md` 和 `HANDOFF.md` 为准。
...
1. **本地优先，隐私优先**
   - 默认不上传 usage、线程、路径、日志或账户数据；用户显式启用的 allowlisted 飞书通知只发送脱敏账号标识、额度窗口、结果与时间。
